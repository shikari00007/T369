...
app.use('/vendor', express_1.default.static(path_1.default.join(__dirname, '..', 'node_modules', 'chart.js', 'dist')));
...
const updateActivitySchema = zod_1.z.object({
    ...
    status: zod_1.z.enum(['pending', 'in_progress', 'done', 'blocked']).optional()
});
const plannerSchema = zod_1.z.object({
    date: zod_1.z.string().date(),
    morning: zod_1.z.array(zod_1.z.string()).max(20),
    afternoon: zod_1.z.array(zod_1.z.string()).max(30),
    night: zod_1.z.array(zod_1.z.string()).max(40)
});
const settingsSchema = zod_1.z.object({
    theme: zod_1.z.enum(['dark', 'light', 'anime', 'cyberpunk'])
});
const taskSchema = zod_1.z
    .object({
    task_type: zod_1.z.enum(['personal', 'professional']),
    title: zod_1.z.string().min(1).max(180),
    description: zod_1.z.string().max(5000).default(''),
    project_name: zod_1.z.string().max(180).nullable().optional(),
    severity: zod_1.z.enum(['high', 'medium', 'low']),
    assigned_to: zod_1.z.coerce.number().int().positive().nullable().optional(),
    category_id: zod_1.z.coerce.number().int().positive().nullable().optional(),
    due_date: zod_1.z.string().date().nullable().optional(),
    ...
    mandays: zod_1.z.coerce.number().min(0).nullable().optional(),
    notes: zod_1.z.string().max(5000).default('')
})
    .superRefine((value, ctx) => {
    if (value.task_type === 'personal' && !value.category_id) {
        ctx.addIssue({ code: zod_1.z.ZodIssueCode.custom, path: ['category_id'], message: 'Personal tasks require a category' });
    }
    if (value.task_type === 'professional' && !value.project_name) {
        ctx.addIssue({ code: zod_1.z.ZodIssueCode.custom, path: ['project_name'], message: 'Professional tasks require a project name' });
    }
});
const teamMemberSchema = zod_1.z.object({
    ...
    status: zod_1.z.enum(['active', 'vacation']).default('active')
});
const vacationSchema = zod_1.z
    .object({
    member_id: zod_1.z.coerce.number().int().positive(),
    start_date: zod_1.z.string().date(),
    end_date: zod_1.z.string().date(),
    reason: zod_1.z.string().max(400).default('')
})
    .refine((value) => (0, dayjs_1.default)(value.end_date).isSame((0, dayjs_1.default)(value.start_date)) || (0, dayjs_1.default)(value.end_date).isAfter((0, dayjs_1.default)(value.start_date)), {
    message: 'End date must be on or after start date',
    path: ['end_date']
});
const reassignmentSchema = zod_1.z.object({
    task_id: zod_1.z.coerce.number().int().positive(),
    from_member: zod_1.z.coerce.number().int().positive().nullable().optional(),
    to_member: zod_1.z.coerce.number().int().positive(),
    reason: zod_1.z.string().max(400).default('')
});
const focusSessionSchema = zod_1.z
    .object({
    duration_minutes: zod_1.z.coerce.number().int().positive().max(720),
    started_at: zod_1.z.string().datetime(),
    completed_at: zod_1.z.string().datetime()
})
    .refine((value) => (0, dayjs_1.default)(value.completed_at).isAfter((0, dayjs_1.default)(value.started_at)), {
...
function toTaskResponse(row) {
    return {
        ...
        overdue: Boolean(row.due_date && (0, dayjs_1.default)(row.due_date).isBefore((0, dayjs_1.default)().startOf('day')) && row.status !== 'completed')
    };
}
async function computeStreaks() {
    const rows = await (0, db_1.query)(`SELECT DISTINCT DATE(completed_at) AS d
     FROM activities
     WHERE completed = TRUE AND completed_at IS NOT NULL
     ORDER BY d DESC`);
    const daySet = new Set(rows.rows.map((r) => (0, dayjs_1.default)(r.d).format('YYYY-MM-DD')));
    let daily = 0;
    let cursor = (0, dayjs_1.default)().startOf('day');
...
async function awardAchievement(code, title, description) {
    const res = await (0, db_1.query)(`INSERT INTO achievements (user_id, code, title, description)
     VALUES ($1, $2, $3, $4)
     ON CONFLICT (user_id, code) DO NOTHING`, [defaultUserId, code, title, description]);
    return (res.rowCount ?? 0) > 0;
}
async function refreshAchievements() {
    const unlocked = [];
    const totalsRes = await (0, db_1.query)(`SELECT
       COUNT(*) FILTER (WHERE completed = TRUE)::int AS completed,
       COALESCE((SELECT SUM(xp) FROM xp_history WHERE user_id = $1), 0)::int AS xp
     FROM activities`, [defaultUserId]);
    ...
    const allCategoryDoneRes = await (0, db_1.query)(`SELECT COUNT(*)::int AS done_categories
     FROM (
       SELECT c.id
       FROM categories c
    ...
    if (completedTasks >= 1 && (await awardAchievement('first_completion', 'First Completion', 'Completed your first 369 activity')))
        unlocked.push('First Completion');
    if (streaks.daily >= 1 && (await awardAchievement('first_streak', 'First Streak', 'Started your first completion streak')))
        unlocked.push('First Streak');
    if (streaks.daily >= 7 && (await awardAchievement('streak_7', '7 Day Streak', 'Maintained a 7-day completion streak')))
        unlocked.push('7 Day Streak');
    if (streaks.daily >= 30 && (await awardAchievement('streak_30', '30 Day Streak', 'Maintained a 30-day completion streak')))
        unlocked.push('30 Day Streak');
    if (completedTasks >= 100 && (await awardAchievement('tasks_100', '100 Tasks Completed', 'Completed 100 activities')))
        unlocked.push('100 Tasks Completed');
    if (doneCategories >= 1 && (await awardAchievement('category_master', 'Category Master', 'Fully completed one life category')))
        unlocked.push('Category Master');
    if (doneCategories >= seedData_1.CATEGORY_SEEDS.length && (await awardAchievement('life_master', 'Life Master', 'Fully completed every category')))
        unlocked.push('Life Master');
    if (totalXp >= 2000 && (await awardAchievement('legend_xp', 'Legend XP', 'Reached Legend level by XP')))
        unlocked.push('Legend XP');
    return unlocked;
}
async function ensureSeedData() {
    for (const category of seedData_1.CATEGORY_SEEDS) {
        const catRes = await (0, db_1.query)(`INSERT INTO categories (name, icon, avatar, color_a, color_b)
       VALUES ($1, $2, $3, $4, $5)
       ON CONFLICT (name)
       DO UPDATE SET icon = EXCLUDED.icon, avatar = EXCLUDED.avatar, color_a = EXCLUDED.color_a, color_b = EXCLUDED.color_b
       RETURNING id`, [category.name, category.icon, category.avatar, category.colorA, category.colorB]);
        const categoryId = catRes.rows[0].id;
        const countRes = await (0, db_1.query)(`SELECT COUNT(*)::int AS total FROM activities WHERE category_id = $1`, [categoryId]);
        const count = Number(countRes.rows[0]?.total || 0);
        if (count >= 18)
            continue;
        await (0, db_1.query)('DELETE FROM activities WHERE category_id = $1', [categoryId]);
        const pushActivities = async (tier, list) => {
            for (let i = 0; i < list.length; i += 1) {
                await (0, db_1.query)(`INSERT INTO activities (category_id, tier, title, position)
           VALUES ($1, $2, $3, $4)`, [categoryId, tier, list[i], i + 1]);
            }
        };
...
async function ensureTaskManagementSchema() {
    const statements = [
    ...
    for (const statement of statements) {
        await (0, db_1.query)(statement);
    }
}
async function syncTeamMemberStatuses() {
    await (0, db_1.query)(`UPDATE team_members tm
     SET status = CASE
...
async function ensureCategoryExists(categoryId) {
    if (!categoryId)
        return;
    const result = await (0, db_1.query)('SELECT id FROM categories WHERE id = $1', [categoryId]);
    if (!result.rows[0]) {
        throw new Error('CATEGORY_NOT_FOUND');
    }
}
async function ensureTeamMemberExists(memberId) {
    if (!memberId)
        return;
    const result = await (0, db_1.query)('SELECT id, status FROM team_members WHERE id = $1', [memberId]);
    if (!result.rows[0] || result.rows[0].status === 'archived') {
        throw new Error('TEAM_MEMBER_NOT_FOUND');
    }
}
async function getDashboardPayload() {
    const categoriesRes = await (0, db_1.query)(`SELECT c.id, c.name, c.icon, c.avatar, c.color_a, c.color_b,
...
async function getTasks(taskType, severity, status, searchText) {
    ...
    const taskRes = await (0, db_1.query)(`SELECT t.id,
            t.task_type,
            t.title,
            t.description,
            t.project_name,
    ...
    if (taskType === 'professional') {
        const teamCountsRes = await (0, db_1.query)(`SELECT status, COUNT(*)::int AS total
       FROM team_members
       GROUP BY status`);
...
app.get('/health', async (_req, res) => {
    const result = await (0, db_1.query)('SELECT 1 AS ok');
...
app.get('/api/categories/:id', async (req, res, next) => {
    try {
        ...
        const catRes = await (0, db_1.query)('SELECT id, name, icon, avatar, color_a, color_b FROM categories WHERE id = $1', [id]);
        if (!catRes.rows[0]) {
            res.status(404).json({ error: 'Category not found' });
            return;
        }
        const activityRes = await (0, db_1.query)(`SELECT id, category_id, tier, title, notes, priority, due_date::text, status, completed
       FROM activities
       WHERE category_id = $1
       ORDER BY tier, position`, [id]);
        const group = { foundation: [], growth: [], mastery: [] };
        for (const activity of activityRes.rows) {
            if (activity.tier === 3)
                group.foundation.push(activity);
            else if (activity.tier === 6)
                group.growth.push(activity);
            else
                group.mastery.push(activity);
        }
...
app.patch('/api/activities/:id', async (req, res, next) => {
    try {
        ...
        const unlocked = await (0, db_1.withTransaction)(async (client) => {
            const currentRes = await client.query('SELECT id, completed, tier FROM activities WHERE id = $1 FOR UPDATE', [id]);
            ...
            if (parsed.data.completed === true && current.completed === false) {
                await client.query('INSERT INTO xp_history (user_id, activity_id, xp) VALUES ($1, $2, $3)', [defaultUserId, id, xpForTier(current.tier)]);
            }
        ...
        const updatedRes = await (0, db_1.query)(`SELECT id, category_id, tier, title, notes, priority, due_date::text, status, completed
...
app.get('/api/tasks', async (req, res, next) => {
    try {
        ...
        const status = req.query.status === 'pending' || req.query.status === 'in_progress' || req.query.status === 'completed' ? req.query.status : undefined;
...
app.post('/api/tasks', async (req, res, next) => {
    try {
        ...
        const insertRes = await (0, db_1.query)(`INSERT INTO tasks (task_type, title, description, project_name, severity, status, due_date, end_date, due_time, assigned_to, category_id, notes, mandays)
       ...
       LEFT JOIN categories c ON c.id = t.category_id
       LEFT JOIN team_members tm ON tm.id = t.assigned_to
       WHERE t.id = $1`, [insertRes.rows[0].id]);
...
app.put('/api/tasks/:id', async (req, res, next) => {
    try {
        ...
        await ensureCategoryExists(payload.category_id);
        await ensureTeamMemberExists(payload.assigned_to);
        const updateRes = await (0, db_1.query)(`UPDATE tasks
       SET task_type = $1,
           title = $2,
           description = $3,
           project_name = $4,
...
app.get('/api/team-members', async (_req, res, next) => {
    try {
        await syncTeamMemberStatuses();
        const rows = await (0, db_1.query)(`SELECT id, name, email, role, status, created_at::text
       FROM team_members
...
app.post('/api/team-members', async (req, res, next) => {
    try {
        ...
        const insertRes = await (0, db_1.query)(`INSERT INTO team_members (name, email, role, status)
       VALUES ($1, $2, $3, $4)
       RETURNING id, name, email, role, status, created_at::text`, [payload.name, payload.email, payload.role, payload.status]);
...
app.put('/api/team-members/:id', async (req, res, next) => {
    try {
        ...
        const updateRes = await (0, db_1.query)(`UPDATE team_members
       SET name = $1,
           email = $2,
           role = $3,
           status = $4
       WHERE id = $5
       RETURNING id, name, email, role, status, created_at::text`, [payload.name, payload.email, payload.role, payload.status, id]);
        if (!updateRes.rows[0]) {
            res.status(404).json({ error: 'Team member not found' });
            return;
        }
...
app.delete('/api/team-members/:id', async (req, res, next) => {
    try {
        ...
        await (0, db_1.query)(`DELETE FROM team_members WHERE id = $1`, [id]);
        res.status(204).send();
    }
    catch (error) {
        next(error);
...
app.get('/api/vacations', async (_req, res, next) => {
    try {
        await syncTeamMemberStatuses();
        const rows = await (0, db_1.query)(`SELECT v.id,
              v.member_id,
...
app.post('/api/vacations', async (req, res, next) => {
    try {
        ...
        const insertRes = await (0, db_1.query)(`INSERT INTO vacations (member_id, start_date, end_date, reason)
       VALUES ($1, $2, $3, $4)
       RETURNING id, member_id, start_date::text, end_date::text, reason, created_at::text`, [parsed.data.member_id, parsed.data.start_date, parsed.data.end_date, parsed.data.reason.trim()]);
        await syncTeamMemberStatuses();
...
app.put('/api/vacations/:id', async (req, res, next) => {
    try {
        ...
        await ensureTeamMemberExists(parsed.data.member_id);
        const updateRes = await (0, db_1.query)(`UPDATE vacations
       SET member_id = $1,
           start_date = $2,
           end_date = $3,
           reason = $4
       WHERE id = $5
       RETURNING id, member_id, start_date::text, end_date::text, reason, created_at::text`, [parsed.data.member_id, parsed.data.start_date, parsed.data.end_date, parsed.data.reason.trim(), id]);
        if (!updateRes.rows[0]) {
...
app.delete('/api/vacations/:id', async (req, res, next) => {
    try {
        ...
        await (0, db_1.query)('DELETE FROM vacations WHERE id = $1', [id]);
        await syncTeamMemberStatuses();
        res.status(204).send();
    }
    catch (error) {
...
app.get('/api/reassignments', async (_req, res, next) => {
    try {
        const rows = await (0, db_1.query)(`SELECT tr.id,
              tr.task_id,
...
app.post('/api/reassignments', async (req, res, next) => {
    try {
        ...
        const result = await (0, db_1.withTransaction)(async (client) => {
            const taskRes = await client.query('SELECT id, task_type, assigned_to FROM tasks WHERE id = $1 FOR UPDATE', [parsed.data.task_id]);
            ...
            if (fromMember !== null) {
                const fromRes = await client.query('SELECT id FROM team_members WHERE id = $1 AND status <> $2', [fromMember, 'archived']);
                if (!fromRes.rows[0]) {
                    throw new Error('TEAM_MEMBER_NOT_FOUND');
                }
            }
            const toRes = await client.query('SELECT id FROM team_members WHERE id = $1 AND status <> $2', [parsed.data.to_member, 'archived']);
            if (!toRes.rows[0]) {
                throw new Error('TEAM_MEMBER_NOT_FOUND');
            }
            await client.query('UPDATE tasks SET assigned_to = $1, updated_at = NOW() WHERE id = $2', [parsed.data.to_member, parsed.data.task_id]);
            const insertRes = await client.query(`INSERT INTO task_reassignments (task_id, from_member, to_member, reason)
         VALUES ($1, $2, $3, $4)
         RETURNING id, task_id, from_member, to_member, reason, created_at::text`, [parsed.data.task_id, fromMember, parsed.data.to_member, parsed.data.reason.trim()]);
        ...
        if (error.message === 'TASK_NOT_PROFESSIONAL') {
        ...
        if (error.message === 'REASSIGNMENT_CONFLICT') {
...
app.get('/api/team-tasks', async (_req, res, next) => {
    try {
        const membersRes = await (0, db_1.query)(`SELECT id, name, email, role, status FROM team_members WHERE status <> 'archived' ORDER BY name ASC`);
        const currentMonth = new Date().toISOString().slice(0, 7); // YYYY-MM
        const tasksRes = await (0, db_1.query)(`SELECT id, assigned_to, title, project_name, status, severity,
...
app.get('/api/vacation-calendar', async (_req, res, next) => {
    try {
        const res2 = await (0, db_1.query)(`SELECT v.id, v.member_id, tm.name AS member_name,
...
app.get('/api/focus-sessions', async (_req, res, next) => {
    try {
        const sessionsRes = await (0, db_1.query)(`SELECT id,
              duration_minutes,
              started_at::text,
              completed_at::text
       FROM focus_sessions
...
app.post('/api/focus-sessions', async (req, res, next) => {
    try {
        ...
        const insertRes = await (0, db_1.query)(`INSERT INTO focus_sessions (duration_minutes, started_at, completed_at)
       VALUES ($1, $2, $3)
       RETURNING id, duration_minutes, started_at::text, completed_at::text`, [parsed.data.duration_minutes, parsed.data.started_at, parsed.data.completed_at]);
...
app.get('/api/life-category/planner', async (req, res, next) => {
    try {
        const date = typeof req.query.date === 'string' && req.query.date ? req.query.date : (0, dayjs_1.default)().format('YYYY-MM-DD');
        const row = await (0, db_1.query)(`SELECT morning, afternoon, night
       FROM planner_entries
       WHERE user_id = $1 AND plan_date = $2`, [defaultUserId, date]);
        res.json({
            date,
...
app.put('/api/life-category/planner', async (req, res, next) => {
    try {
        ...
        await (0, db_1.query)(`INSERT INTO planner_entries (user_id, plan_date, morning, afternoon, night)
       VALUES ($1, $2, $3::jsonb, $4::jsonb, $5::jsonb)
       ON CONFLICT (user_id, plan_date)
       DO UPDATE SET morning = EXCLUDED.morning,
                     afternoon = EXCLUDED.afternoon,
                     night = EXCLUDED.night,
                     updated_at = NOW()`, [defaultUserId, date, JSON.stringify(morning), JSON.stringify(afternoon), JSON.stringify(night)]);
...
app.delete('/api/life-category/planner', async (req, res, next) => {
    try {
        const date = typeof req.query.date === 'string' && req.query.date ? req.query.date : (0, dayjs_1.default)().format('YYYY-MM-DD');
        await (0, db_1.query)(`DELETE FROM planner_entries
       WHERE user_id = $1 AND plan_date = $2`, [defaultUserId, date]);
...
app.get('/api/life-category/planner/history', async (req, res, next) => {
    try {
        ...
        const rows = await (0, db_1.query)(`SELECT plan_date::text
       FROM planner_entries
       WHERE user_id = $1
       ORDER BY plan_date DESC
       LIMIT $2`, [defaultUserId, limit]);
...
app.get('/api/progress/charts', async (_req, res, next) => {
    try {
        const dailyRes = await (0, db_1.query)(`SELECT DATE(completed_at)::text AS d, COUNT(*)::int AS count
       FROM activities
       ...
       ORDER BY d ASC`);
        ...
        for (let i = 13; i >= 0; i -= 1) {
            const d = (0, dayjs_1.default)().subtract(i, 'day').format('YYYY-MM-DD');
            labels.push((0, dayjs_1.default)(d).format('DD MMM'));
            values.push(dailyMap.get(d) || 0);
        }
        const catRes = await (0, db_1.query)(`SELECT c.name,
              COUNT(*) FILTER (WHERE a.completed = TRUE)::int AS done,
              COUNT(*)::int AS total
       FROM categories c
       JOIN activities a ON a.category_id = c.id
...
app.get('/api/completed-tasks', async (_req, res, next) => {
    try {
        const personalProfessionalRes = await (0, db_1.query)(`SELECT t.id,
              t.task_type,
              t.title,
              t.project_name,
              tm.name AS assigned_name,
       ...
       ORDER BY t.updated_at DESC`);
        const lifeRes = await (0, db_1.query)(`SELECT a.id,
              a.title,
              c.name AS category_name,
              a.completed_at::text
       FROM activities a
       ...
       ORDER BY a.completed_at DESC`);
        ...
        ].sort((a, b) => (0, dayjs_1.default)(b.completed_at).valueOf() - (0, dayjs_1.default)(a.completed_at).valueOf());
        res.json({
            items,
            stats: {
...
app.get('/api/settings', async (_req, res, next) => {
    try {
        const row = await (0, db_1.query)('SELECT theme FROM settings WHERE user_id = $1', [defaultUserId]);
...
app.put('/api/settings', async (req, res, next) => {
    try {
        ...
        await (0, db_1.query)(`INSERT INTO settings (user_id, theme)
       VALUES ($1, $2)
       ON CONFLICT (user_id)
       DO UPDATE SET theme = EXCLUDED.theme, updated_at = NOW()`, [defaultUserId, parsed.data.theme]);
        res.json({ status: 'saved' });
    }
    catch (error) {
        next(error);
